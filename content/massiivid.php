...
<div id="u2">
    ...
    Autod massiiviandmed võta lehel - https://www.metshein.com/unit/php-massiivid-ulesanne-5/
...
echo "<ol>";
for ($i = 0; $i < count($auto_mark); $i++) {
    echo "<li><p>$auto_mark[$i]</p></li>";
}
echo "</ol>";
echo "<br>";

echo "<h3>Autode arv (count)</h3>";
echo "Autode arv : ".count($auto_mark)." tk";
echo "<br>";

echo "<h3>Kontrolli, kas massiviid on ühepikkused</h3>";
if (count($auto_mark) == count($vin_num)) {
    echo "Massiivid on ühepikulised";
}
else {
    echo "Massiivid ei ole ühipikulised";
}
echo "<br>";
echo "<h3>Eraldi Toyotade ja Audide arv nimekirjas</h3>";
$toyota_arv=0;
$audi_arv=0;
for ($i = 0; $i < count($auto_mark); $i++) {
    if ("Toyota"== $auto_mark[$i]) {
        $toyota_arv++;
    }
    if ("audi"== $auto_mark[$i]) {
        $audi_arv++;
...
echo "Toyoyta arv: ".$toyota_arv."<br>";
echo "Audi arv: ".$audi_arv."<br>";
echo "<h3>Väljasta VIN koodid,  mille märkide arv on väiksem kui 17</h3>";
for ($i = 0; $i < count($vin_num); $i++) {
    if (strlen($vin_num[$i]) < 17) {
        echo $auto_mark[$i]."-".$vin_num[$i]."<br>";
    }
}
?>
</div>
<div id="u3">
    <h2>Ülesanne 3</h2>
    Kuva /img massiivist kolmas pilt
<?php
...
echo "<br>";
for ($i = 0; $i < count($pildiArray); $i++) {
    echo "<img src='content/img/$pildiArray[$i]' width='100px' alt='pilt'><br>";
}
?>
</div>
