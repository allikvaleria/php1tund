...
?>
    ...
    <table border="1">
        ...
        while ($paring->fetch()) {
            ...
            echo "<input type='hidden' name='muuda_punktid_id' value='$id'>";
            echo "<select name='punktid'>";
            for ($i = 0; $i <= 5; $i++) {
                $selected = ($punktid == $i) ? 'selected' : '';
                echo "<option value='$i' $selected>$i</option>";
            }
            echo "</select>";
            echo "<input type='submit' class='link-button' value='Muuda punktid'>";
            echo "</form>";
            echo "</td>";
